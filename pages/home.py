...
Keywords_text = [
    "Ce tableau de bord est focalisé sur les mot clés. Abordons d’abord le graphique principal. Nous pouvons choisir une année pour cibler des périodes précises. Ce premier graphe est un graphique en barres représentant le nombre d’articles par jour.",
    html.Br(),
    "Lorsque l’on clique sur une barre, nous obtenons deux graphiques supplémentaires. Un premier qui est également un graphique en barres représentant les 5 mots clés les plus utilisés à cette date. Puis un deuxième graphique en lignes (line chart). Celui représente le nombre de fois que les mots clés ont été utilisé lors de cette même année. Cela permet de se rendre compte si c’est un mot clé utilisé fréquemment ou non. Nous pouvons alors emmètre des hypothèses quant aux évènements qui ont engendrés cela. Par exemple, pour le 8/10/2020 (avec le plus petit fichier), nous obtenons des mots clés tels que « Mali », « otage », « Sophie » et « libération » et en analysant le graphique en lignes, nous pouvons conclure que Sophie a été libéré ce jour là.",
    html.Br(),
...
Organizations_text = [
    ...
    html.Br(),
    "Il est par ailleurs possible de cliquer sur une barre de ce graphique pour afficher de nouveaux résultats sur les deux autres. Un pie chart ainsi qu'un tableau s'affiche alors, le premier affiche les organisations les plus citées en même temps que l'organisation sélectionnée (10 organisations maximum). Ce graphique nous permet par exemple d'observer que les groupes de l'état islamique sont la plupart du temps citée les uns avec les autres et représente la plupart du temps à eux seul 50% des organisations présente lorsque l'une d'elle est citée",
    html.Br(),
...
Conclusion_text = [
    ...
    html.Br(),
    "Enfin, voici un schéma de notre application avec les pages différentes ainsi que les callback.",
    ]
